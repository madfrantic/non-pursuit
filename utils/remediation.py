...
This repo's docs/SAFETY_BOUNDARIES.md designates demand-letter content a Human Validation
...
# Validation Zone in docs/SAFETY_BOUNDARIES.md. The four statutory letters carry that
...
def _payload(*, exposure_type, name, handle, url, confidence, band, contact,
    ...
    reasons = []
    if not signed_off:
        reasons.append("template awaiting human sign-off (see docs/SAFETY_BOUNDARIES.md HVZ)")
...
def _caveats(scored: dict, payloads: list) -> list:
    ...
    if unsigned:
        caveats.append(
            ...
            "human sign-off this repo's docs/SAFETY_BOUNDARIES.md requires for legally binding text. "
